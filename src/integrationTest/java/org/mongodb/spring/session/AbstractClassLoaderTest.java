...
public abstract class AbstractClassLoaderTest<T> extends AbstractITest {
    ...
    void verifyContainerClassLoaderLoadedIntoConverter() {
        ...
        Field mongoSessionConverterField =
                ReflectionUtils.findField(this.sessionRepository.getClass(), "mongoSessionConverter");
        ReflectionUtils.makeAccessible(mongoSessionConverterField);
        AbstractMongoSessionConverter sessionConverter = (AbstractMongoSessionConverter)
                ReflectionUtils.getField(mongoSessionConverterField, this.sessionRepository);
        ...
        DeserializingConverter deserializingConverter = (DeserializingConverter)
                extractField(JdkMongoSessionConverter.class, "deserializer", jdkMongoSessionConverter);
        DefaultDeserializer deserializer = (DefaultDeserializer)
                extractField(DeserializingConverter.class, "deserializer", deserializingConverter);
